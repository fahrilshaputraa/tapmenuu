...
def make_owner(email=None, password='testpass123', restaurant_name='Warung Test'):
    ...
    profile = UserProfile.objects.create(user=user, restaurant=restaurant, role=Role.OWNER)
...
class EmailBackendTest(TestCase):
    ...
    def setUp(self):
        ...
        restaurant = Restaurant.objects.create(
            name='Test Restaurant', slug='test-restaurant-abc', is_active=True,
        )
        UserProfile.objects.create(user=self.user, restaurant=restaurant, role=Role.OWNER)

    def test_login_with_email_succeeds(self):
        logged_in = self.client.login(username='test@example.com', password='testpass123')
        self.assertTrue(logged_in)

    def test_login_with_wrong_email_fails(self):
        logged_in = self.client.login(username='wrong@example.com', password='testpass123')
...
class UserProfileCreationTest(TestCase):
    ...
    def test_register_duplicate_email_rejected(self):
        User.objects.create_user(
            username='existing_abc', email='dup@example.com',
...
class RoleCheckTest(TestCase):
    ...
    def setUp(self):
        self.owner, self.restaurant, self.owner_profile = make_owner(email='owner@test.com')
        self.kasir_user, self.kasir_profile = make_employee(
            'kasir1@test.com', Role.KASIR, self.restaurant,
        )
        self.client = Client()
...
class EmployeeCreateTest(TestCase):
    ...
    def test_create_employee_duplicate_email_shows_error(self):
        User.objects.create_user(
            username='existing_abc', email='existing@warung.com',
            password='pass', is_staff=True,
        )
        response = self.client.post(reverse('employee_create'), {
    ...
    def test_kasir_cannot_create_employee(self):
        kasir_user, _ = make_employee('kasir_creator@test.com', Role.KASIR, self.restaurant)
...
class EmployeeUpdateTest(TestCase):
    ...
    def setUp(self):
        ...
        self.staff_user, self.staff_profile = make_employee(
            'staff1@test.com', Role.KASIR, self.restaurant,
        )
        self.client.login(username='owner@test.com', password='testpass123')
...
class EmployeeDeleteTest(TestCase):
    ...
    def setUp(self):
        self.owner, self.restaurant, self.owner_profile = make_owner(email='owner@test.com')
        self.staff_user, self.staff_profile = make_employee(
            'staff_del@test.com', Role.ADMIN, self.restaurant,
        )
        self.client.login(username='owner@test.com', password='testpass123')
...
class CrossRestaurantAccessTest(TestCase):
    ...
    def setUp(self):
        self.owner_a, self.restaurant_a, _ = make_owner(
            email='owner_a@test.com', restaurant_name='Warung A',
        )
        self.owner_b, self.restaurant_b, _ = make_owner(
            email='owner_b@test.com', restaurant_name='Warung B',
        )
        self.staff_b, self.profile_b = make_employee(
            'staff_b@test.com', Role.KASIR, self.restaurant_b,
        )
